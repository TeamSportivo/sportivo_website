...
const SHEET_CSV_URL =
  "https://docs.google.com/spreadsheets/d/1mBh8PNocqgQiPJyznIG7acqubcv8o1tiDbWPFJlthZY/export?format=csv";

// Manual photo overrides by lowercase name (when sheet photo is wrong/missing)
const PHOTO_OVERRIDES = {
  // "ratul kayal":
  //   "https://drive.google.com/file/d/1jSd6_kI0pM_QOIzv7UlOB6L4OJHceODv/view?usp=sharing",
};
...
function toDirectImageUrl(url) {
  ...
  if (fileId) {
    // Drive thumbnail endpoint is more reliable for form-uploaded photos
    // than lh3.googleusercontent.com/d/... (which often fails in-browser)
...
// Display order for key mentor roles (lower = earlier)
function mentorRoleRank(role) {
  ...
  if (r.includes("badminton head")) return 5;
  return null;
}

function mentorSortKey(member) {
  const rank = mentorRoleRank(member.mentorRole);
  if (rank != null) return rank;
  const order = Number.isFinite(member.mentorOrder) ? member.mentorOrder : 9999;
  return 1000 + order;
}
  ...
  try {
    ...
    const members = data
      .map((row, idx) => {
        ...
        return {
          ...
          category,
          instagram: formatAndValidateSocialUrl(get("instagram"), "instagram"),
          facebook: formatAndValidateSocialUrl(get("facebook"), "facebook"),
          linkedin: formatAndValidateSocialUrl(get("linkedin"), "linkedin"),
        };
      })
      .filter(Boolean);
    ...
    const grouped = {
      mentor: unique
        ...
        .sort((a, b) => {
          const diff = mentorSortKey(a) - mentorSortKey(b);
