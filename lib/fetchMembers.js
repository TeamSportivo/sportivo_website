...
const SHEET_CSV_URL =
  "https://docs.google.com/spreadsheets/d/1E2dtfET-qywObQpxm2Py8hDqGByzIXKPGyeohwI0WsY/export?format=csv";
...
function mentorRoleRank(role) {
  ...
  if (r.includes("cricket head")) return 6;
  if (r.includes("football head")) return 7;
  if (r.includes("table tennis head") || r.includes("tabletennis head"))
    return 8;
  if (r.includes("chess head")) return 9;
  if (r.includes("handball head")) return 10;
  if (r.includes("event head")) return 11;
  return null;
}
